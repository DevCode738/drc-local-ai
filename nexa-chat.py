#!/usr/bin/env python3
"""
NEXA LOCAL AI — Terminal AI with reasoning, web search, shell, skills
"""
...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS sessions (
        ...
        title TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP)""")
    c.execute("""CREATE TABLE IF NOT EXISTS skills (
        id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT UNIQUE NOT NULL,
        code TEXT NOT NULL, description TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)""")
...
def save_msg(role, content, session_id="default"):
    ...
    c.execute("""INSERT INTO sessions (session_id, title, updated_at) VALUES (?, ?, CURRENT_TIMESTAMP)
        ON CONFLICT(session_id) DO UPDATE SET updated_at=CURRENT_TIMESTAMP""", (session_id, content[:50]))
    conn.commit(); conn.close()
...
def get_response_with_reasoning(user_msg, context, use_reasoning=True):
    """Get response. If reasoning model available, use it for thinking then qwen for final."""
    ...
    # If reasoning model available and user wants deep thinking
    if has_r1 and use_reasoning and ("why" in user_msg.lower() or "how" in user_msg.lower() or "explain" in user_msg.lower() or len(user_msg) > 50):
        ...
        r1_msgs = [{"role": "system", "content": "Think step by step. Provide detailed reasoning inside <think></think> tags, then give the final concise answer."}]
        ...
        # If final is too short, get better answer from qwen with reasoning context
        if len(final) < 20:
...
def detect_tool(user_input):
    """Detect if user wants web search, shell, or skill."""
...
def main():
    ...
    if not check_model(models, MODEL):
        ...
        print_sys(f"Pull: ollama pull {MODEL}")
        sys.exit(1)

    print_ok(f"Ollama ready. Model: {MODEL}")
    if check_model(models, REASONING_MODEL):
        print_ok(f"Reasoning engine: {REASONING_MODEL}")
    time.sleep(0.2)
    ...
    while True:
        ...
        if user_input.lower() in ["/exit", "/quit", "exit", "quit"]:
            ...
            if not skills: print_sys("No skills saved yet. Use 'learn <description>' to teach NEXA.")
            else:
...
""")
        ...
        if tool_type == "search":
            ...
            # Generate skill code
            learn_prompt = f"Write a Python script that {desc}. The script should store its result in a variable called 'result'. Only output valid Python code, no explanations."
